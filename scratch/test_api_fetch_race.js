...
// (Only Mock, 0 External Network Requests)
...
async function runUnitTest() {
  ...
  console.log(
    "🧪 [단위 테스트] HTTP API 캔들스틱 도착 순서 역전(Out-of-order) 침범 테스트",
  );
  console.log(
    "========================================================================",
  );
  ...
  console.log(
    "  2) 100$ 코인(AAVE) 20ms 후 빠르게 클릭 -> API 요청 (지연시간 30ms)",
  );
  ...
  if (currentRenderedPrice === 0.1) {
    console.log(
      "  🚨 [침범 참사 발생!] 느린 0.1$ API 응답이 빠른 100$ 차트를 덮어씌움 (FAIL)",
    );
  } else {
  ...
  console.log(
    "[TEST 2] Fetch Guard(fetchId / snapshotAsset 검증) 적용 후 테스트:",
  );

  const storeProtected = new MockChartStore();
  globalFetchSeq = 0;

  const p3 = fetchHistory_ProtectedCode(
    storeProtected,
    "DOGE",
    100,
    dogeCandles,
  ...
  await sleep(20);
  const p4 = fetchHistory_ProtectedCode(
    storeProtected,
    "AAVE",
    30,
    aaveCandles,
  ...
  console.log(
    `  • 사용자 선택 코인 : ${protectedTargetAsset} (기대 가격: 100$)`,
  );
