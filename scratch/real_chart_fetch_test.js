...
async function testRealCode() {
  ...
  store.candleSeries = {
    setData: (data) => {
      console.log(
        `  🎯 [store.candleSeries.setData 호출됨!] 종가: ${data[data.length - 1]?.close}$`,
      );
      chartSeriesLog.push({
        time: Date.now(),
  ...
  console.log(
    "🔥 [실제 static/chart_fetch.js 및 static/_store.js 직접 import 실행 검증]",
  );
  console.log(
    "🔒 외부 API 요청 차단 여부: 100% Mock Intercept (실제 API 호출 0회)",
  );
  console.log(
    "========================================================================",
  );
  ...
  console.log(
    "1. fetchHistory('DOGE') 실제 프로젝트 함수 호출 (가상 지연 80ms)...",
  );
  ...
  console.log(
    "2. 15ms 후 fetchHistory('AAVE') 실제 프로젝트 함수 호출 (가상 지연 25ms)...",
  );
  ...
  chartSeriesLog.forEach((entry, idx) => {
    console.log(
      `  [SetData #${idx + 1}] 차트에 주입된 캔들 종가: ${entry.lastClose}$ (데이터 수: ${entry.dataLength})`,
    );
  });
