...
def _aggregate_binance_market(
    ...
    if DUPLICATED_LIST and final_ucid:
        for k, v in DUPLICATED_LIST.items():
            if (
                len(v) >= 4
                and str(v[0]) == str(final_ucid)
                and "binance" in v[3].lower()
            ):
...
def _resolve_upbit_match(
    base, final_ucid, upbit_krw_set, upbit_data, REVERSE_LOOKUP, DUPLICATED_LIST
):
...
def _resolve_bithumb_match(
    base, target_up_base, final_ucid, bithumb_krw_set, REVERSE_LOOKUP, DUPLICATED_LIST
):
...
def build_binance_row(
    ...
    raw_symbol, base, is_stock = _resolve_base_and_stock_type(
        ticker, b_info, binance_data
    )
    is_updated = False
    display_name, explicit_name = _resolve_display_name(
        is_stock, raw_symbol, base, REVERSE_LOOKUP
    )
    ...
    if exact_spot_ticker and exact_spot_ticker != raw_symbol:
        spot_disp, _ = _resolve_display_name(
            is_stock, exact_spot_ticker, exact_spot_ticker, REVERSE_LOOKUP
        )
    ...
    upbit_direct_match = _resolve_upbit_match(
        base, final_ucid, upbit_krw_set, upbit_data, REVERSE_LOOKUP, DUPLICATED_LIST
    )
    ...
    bithumb_direct_match, target_bi_base = _resolve_bithumb_match(
        base,
        target_up_base,
        final_ucid,
        bithumb_krw_set,
    ...
    row = {
        ...
        "Change_24h_Bybit": float(
            ...
            or (
                bybit_data.get(target_up_base, {}).get("change_24h")
                if target_up_base
                else 0.0
            )
            or 0.0
        ...
        "Change_24h_Bybit_Futures": float(
            bybit_data.get(raw_symbol, {}).get("futures_change_24h")
            or bybit_data.get(base, {}).get("futures_change_24h")
            or bybit_data.get(raw_symbol, {}).get("change_24h")
            or bybit_data.get(base, {}).get("change_24h")
            or 0.0
        ),
        "Change_Today_Bybit": float(
            ...
            or (
                bybit_data.get(target_up_base, {}).get("change_today")
                if target_up_base
                else None
            )
            or bybit_data.get(raw_symbol, {}).get("change_24h")
        ...
        "Change_Today_Bybit_Futures": float(
            bybit_data.get(raw_symbol, {}).get("futures_change_today")
            or bybit_data.get(base, {}).get("futures_change_today")
            or bybit_data.get(raw_symbol, {}).get("change_today")
            or bybit_data.get(base, {}).get("change_today")
            or bybit_data.get(raw_symbol, {}).get("futures_change_24h")
            or bybit_data.get(base, {}).get("futures_change_24h")
            or bybit_data.get(raw_symbol, {}).get("change_24h")
            or bybit_data.get(base, {}).get("change_24h")
            or 0.0
