...
def build_cmc_lookup_lists(binance_data, upbit_krw_set, MAPPING_DATA):
    id_lookup, sym_lookup = [], []
    asset_to_lookup_key = {}

    binance_base_set = {
        utils.get_pure_base_asset(t.replace("USDT", "")).upper()
        for t in binance_data.keys()
    }
    ...
    def process_asset(a, exchange_tag):
        ...
        if not base or "_" in base or re.search(r"\d{6}$", base):
            return

        # 이름표 및 별칭 확정 (요청대로 BTC/ETH 자르기는 패스하므로 a를 우선 사용)
        resolved_tag = exchange_tag
        if exchange_tag == "BINANCE":
            ticker = f"{a}USDT"
            b_info = binance_data.get(ticker)
            is_asset_stock = False
            if b_info:
                u_type = (
                    str(b_info.get("underlying_type", ""))
                    if isinstance(b_info, dict)
                    else ""
                )
                c_type = (
                    str(b_info.get("contract_type", ""))
                    if isinstance(b_info, dict)
                    else ""
                )
                is_asset_stock = ("EQUITY" in u_type) or (c_type == "TRADIFI_PERPETUAL")

            # spot tokenized stock detection (e.g. MSTRB)
            if not is_asset_stock and a.endswith("B") and len(a) > 2:
                cand_futures = f"{a[:-1]}USDT"
            ...
            if is_asset_stock:
                resolved_tag = "BINANCE_STOCK"

        lookup_name = f"{a.upper()}_{resolved_tag}"
...
def execute_cmc_requests(id_lookup, sym_lookup, api_key=None):
    ...
    if id_lookup:
        quote_tasks.append(
            (url, headers, {"id": ",".join(id_lookup), "convert": "USD"})
        )
    ...
    for i in range(0, len(sym_list), 200):
        ...
        if chunk:
            quote_tasks.append(
                (url, headers, {"symbol": ",".join(chunk), "convert": "USD"})
            )
    ...
    for res in results:
        ...
        for k, v in res["data"].items():
            ...
            for info in items:
                if not info or "quote" not in info:
                    continue
                ...
                asset_info = {
                    ...
                    "ucid": ucid_str,
                    "chain_symbol": (
                        info.get("platform", {}).get("symbol")
                        if info.get("platform")
                        else info.get("symbol", "")
                    ),
                }
                ...
                if info.get("symbol"):
                    ...
                    if "derivatives" in name.lower() or "stock" in name.lower():
                        ...
                        if f"{sym_upper}_COIN" not in market_data_map:
                            market_data_map[f"{sym_upper}_COIN"] = asset_info

                    # 일반 티커로도 백업 등록 (가장 순위가 높은 첫 번째 매칭 자산이 선점하도록 처리)
