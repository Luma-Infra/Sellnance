...
def load_mapping_data():
    ...
    try:
        ...
        if _LAST_VALID_MAPPING_DATA:
            print("🛡️ [SAFEGUARD] mapping.json 읽기 이상 감지 -> 메모리 백업 족보로 즉각 복구 반환")
...
def save_mapping_data(mapping_data):
    ...
    try:
        ...
        if len(ticker_data) < 10 and len(dup_list) < 5:
            print("🚨 [CRITICAL SAFEGUARD] TICKER_DATA 또는 DUPLICATED_LIST가 비정상적으로 비어있어 mapping.json 덮어쓰기를 강제 차단합니다!")
            return False

        # 🚀 [요청] TICKER_DATA를 A-Z 알파벳 순으로 깔끔하게 정렬!
        if "TICKER_DATA" in mapping_data and isinstance(mapping_data["TICKER_DATA"], dict):
