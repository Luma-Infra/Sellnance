...
def is_valid_cmc_key_format(key: str | None) -> bool:
...
def _prune_user_cmc_caches():
    """유저별 CMC 캐시 개수 상한 관리 및 만료 캐시 자동 퇴출 (메모리 보호)"""
...
# 서버 로드 시 단일 통합 스케줄러 즉시 실행 (스레드 1개로 중복 실행 방지)
...
def get_cached_data(force_reload=False, silent_mode=False, user_api_key=None):
    ...
    if user_api_key and is_valid_cmc_key_format(user_api_key):
        ...
        # 4. 고정 해시 버킷 락 획득 (Race Condition & Lock 삭제 누수 방지)
