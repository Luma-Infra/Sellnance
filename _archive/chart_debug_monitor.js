(() => {
      ...
      seriesHTML += `
        <div style="background: rgba(255, 255, 255, 0.03); border: 1px solid rgba(255, 255, 255, 0.08); padding: 8px; border-radius: 6px; margin-bottom: 6px;">
          <div style="display: flex; justify-content: space-between; font-weight: bold; color: #ffbc00; font-size: 11.5px; border-bottom: 1px solid rgba(255,255,255,0.05); padding-bottom: 4px; margin-bottom: 6px;">
            <span>📈 ${name}</span>
            <span style="color: ${errCount > 0 ? '#ff6b6b' : '#10b981'};">오류: ${errCount}건</span>
          </div>
  ...
  // 5. 차트 시리즈 래핑 헬퍼 함수
  const wrapSeriesInstance = (series, seriesName) => {
  ...
  const initializeHook = () => {
    // 6-1. 기존 생성된 시리즈 래핑
    if (window.store) {
      if (window.store.candleSeries) wrapSeriesInstance(window.store.candleSeries, "CandleSeries (가격)");
      if (window.store.leftScaleSeries) wrapSeriesInstance(window.store.leftScaleSeries, "LeftScaleSeries (등락률)");
      if (window.store.volumeSeries) wrapSeriesInstance(window.store.volumeSeries, "VolumeSeries (거래량)");
      if (window.store.kimchiSeries) wrapSeriesInstance(window.store.kimchiSeries, "KimchiSeries (김프)");
    }
