...
async function tickAlphaStream() {
  ...
  // 현재 테이블에 알파 코인이 하나라도 있는지 빠른 검사
  const currentRows = store.currentTableData || [];
  const alphaRows = currentRows.filter(
    (r) => r && (r.Binance_Alpha === "O" || r.is_alpha)
  );

  if (alphaRows.length === 0) return; // 화면에 알파 코인이 없으면 통신 생략
  ...
  try {
    ...
    for (let i = 0; i < alphaRows.length; i++) {
      ...
      const openUtc0 = row.spot_utc0_open_Raw || row.utc0_open_Raw || 0;
      if (openUtc0 > 0 && newPrice > 0) {
        const todayChg = ((newPrice - openUtc0) / openUtc0) * 100;
        row.Change_Today_Raw = todayChg;
        row.Change_Today_Spot = todayChg;
        row.Change_Today_Binance = todayChg;
        if (typeof window.formatChangePercent === "function") {
          row.Change_Today = window.formatChangePercent(todayChg);
        } else {
          row.Change_Today = `${todayChg >= 0 ? "+" : ""}${todayChg.toFixed(2)}%`;
        }
      }
...
export function initAlphaStreamPipeline() {
  ...
  console.log("[Alpha Pipeline] 💎 독립 알파 실시간 스트림 파이프라인 가동 완료");
}
